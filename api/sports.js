...
module.exports=async function handler(req,res){
 ...
 try{
  if(String(q.motorsport||'')==='f1'){
   ...
   if(view==='calendar'){const a=await fetchJSON(base+'meetings?year='+year);return res.status(200).json({sport:'f1',view,items:(a||[]).filter(x=>!x.is_cancelled).map(x=>({name:x.meeting_name,circuit:x.circuit_short_name,country:x.country_name,start:x.date_start,end:x.date_end,flag:x.country_flag,circuitImage:x.circuit_image}))})}
