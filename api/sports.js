...
module.exports=async function handler(req,res){
 ...
 try{
  if(String(q.motorsport||'')==='f1'){
   ...
   if(view==='calendar'){const a=await fetchJSON(base+'meetings?year='+year),now=Date.now();const items=(a||[]).filter(x=>!x.is_cancelled&&new Date(x.date_end||x.date_start).getTime()>=now).sort((a,b)=>new Date(a.date_start)-new Date(b.date_start)).map(x=>({name:x.meeting_name,circuit:(String(x.meeting_name||'').includes('Bahrain')&&String(x.country_name||'').toLowerCase().includes('malaysia')?'Sepang International Circuit':x.circuit_short_name),country:x.country_name,start:x.date_start,end:x.date_end,flag:x.country_flag,circuitImage:x.circuit_image}));return res.status(200).json({sport:'f1',view,items})}
