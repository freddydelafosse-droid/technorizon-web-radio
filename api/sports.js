...
module.exports=async function handler(req,res){
 ...
 if(String(param('motorsport')||'')==='f1'){try{const base='https://api.openf1.org/v1/';const view=String(param('view')||'calendar');const year=new Date().getUTCFullYear();
 ...
 if(String(param('motorsport')||'')==='motogp'){try{const base='https://api.motogp.pulselive.com/motogp/v1/';const year=new Date().getUTCFullYear(),view=String(param('view')||'calendar');const seasons=await fetch(base+'results/seasons').then(r=>r.json());const season=(seasons||[]).find(x=>Number(x.year)===year);if(!season?.id)throw Error('season');const cats=await fetch(base+'results/categories?seasonUuid='+encodeURIComponent(season.id)).then(r=>r.json());const cat=(cats||[]).find(x=>Number(x.legacy_id)===3||String(x.name||'').toLowerCase().includes('motogp'));if(!cat?.id)throw Error('category');
 ...
 const leagueView=String(param('view')||'').toLowerCase();
 if(tableLeague){try{const base=('https://www.thesportsdb.com/api/v1/json/'+(process.env.THESPORTSDB_API_KEY||'3')+'/');
  ...
  if(leagueView==='results'||leagueView==='upcoming'){const endpoint=leagueView==='results'?'eventspastleague.php?id=':'eventsnextleague.php?id=';const r=await timedFetch(base+endpoint+encodeURIComponent(tableLeague));if(!r.ok)throw Error('provider');const d=await r.json();let events=(d.events||[]).map(eventOut);events.sort((a,b)=>leagueView==='results'?String(b.time).localeCompare(String(a.time)):String(a.time).localeCompare(String(b.time)));return res.status(200).json({league:param('league'),view:leagueView,events:events.slice(0,12)})}
  const r=await timedFetch(base+'lookuptable.php?l='+encodeURIComponent(tableLeague));if(!r.ok)throw Error('provider');const d=await r.json();const table=(d.table||[]).map(tableRow).sort((a,b)=>a.rank-b.rank);return res.status(200).json({league:param('league'),table})}catch(e){return res.status(502).json({table:[],events:[],error:'sports_provider_unavailable'})}}
 const c=countryNames[String(param('country')||'fr').toLowerCase()]||'France';
 const s=sportNames[String(param('sport')||'football').toLowerCase()]||'Soccer';
 try{
