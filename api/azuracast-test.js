...
Ce qu'il faut apprendre de ces exemples : réaction spontanée au morceau, adresse directe à l'auditeur, complicité, humour et exagération légère, teasing de ce qui arrive, rythme oral, relance courte, identité Technorizon. Mélange seulement 2 ou 3 de ces mécanismes par intervention. Les expressions « furax », voisins, tympans, « ça percute », « ça va taper fort » et « montez l'son » sont des épices rares : ne jamais en faire des gimmicks, ne jamais répéter une expression récemment utilisée. Jaya garde sa propre personnalité et invente des formulations nouvelles.
`;
 try{
  const r=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:"Bearer "+key,"Content-Type":"application/json"},body:JSON.stringify({
   model:"gpt-5-mini",
