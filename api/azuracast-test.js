...
function jayaTooGeneric(text){
 ...
 return jayaRecent.slice(-100).some(old=>{const na=normJaya(old),nb=normJaya(text);if(na===nb)return true;const a=new Set(na.split(" ").filter(x=>x.length>3)),b=nb.split(" ").filter(x=>x.length>3);if(!a.size||!b.length)return false;const common=b.filter(x=>a.has(x)).length;return common/Math.min(a.size,b.length)>=0.28});
}
...
async function weatherBulletin(){const pool=[["Lille",50.6292,3.0573],["Paris",48.8566,2.3522],["Strasbourg",48.5734,7.7521],["Nantes",47.2184,-1.5536],["Bordeaux",44.8378,-0.5792],["Lyon",45.764,4.8357],["Marseille",43.2965,5.3698],["Rouen",49.4431,1.0993],["Rennes",48.1173,-1.6778],["Brest",48.3904,-4.4861],["Caen",49.1829,-0.3707],["Amiens",49.8941,2.2958],["Reims",49.2583,4.0317],["Dijon",47.322,5.0415],["Orléans",47.903,1.9093],["Tours",47.3941,0.6848],["Poitiers",46.5802,0.3404],["Limoges",45.8336,1.2611],["Clermont-Ferrand",45.7772,3.087],["Grenoble",45.1885,5.7245],["Toulouse",43.6047,1.4442],["Montpellier",43.6108,3.8767],["Nice",43.7102,7.262],["Perpignan",42.6887,2.8948]];const day=new Intl.DateTimeFormat("en-CA",{timeZone:"Europe/Paris",year:"numeric",month:"2-digit",day:"2-digit"}).format(new Date());const seed=hash(day+"weather-cities");const cities=Array.from({length:7},(_,i)=>pool[(seed+i*7)%pool.length]);const settled=await Promise.allSettled(cities.map(async([city,latitude,longitude])=>{const u=new URL("https://api.open-meteo.com/v1/forecast");u.searchParams.set("latitude",latitude);u.searchParams.set("longitude",longitude);u.searchParams.set("daily","weather_code,temperature_2m_max,precipitation_probability_max");u.searchParams.set("timezone","Europe/Paris");u.searchParams.set("forecast_days","1");const r=await fetch(u);if(!r.ok)throw new Error("Weather "+city);const j=await r.json();return{city,max:Math.round(j.daily.temperature_2m_max[0]),rain:Math.round(j.daily.precipitation_probability_max[0]||0),code:Number(j.daily.weather_code[0]||0)}}));const data=settled.filter(x=>x.status==="fulfilled").map(x=>x.value);if(data.length<4)throw new Error("Weather data insufficient");const ordered=data.slice().sort((a,b)=>hash(day+a.city+"order")-hash(day+b.city+"order"));const wet=ordered.filter(x=>x.rain>=50).map(x=>x.city);const temps=ordered.map((x,i)=>(i===ordered.length-1?"et ":"")+x.max+" degrés à "+x.city).join(", ").replace(", et "," et ");const skyRows=ordered.slice(0,3).map(x=>({city:x.city,sky:weatherSky(x.code)}));const skyGroups=new Map();for(const x of skyRows){const a=skyGroups.get(x.sky)||[];a.push(x.city);skyGroups.set(x.sky,a)}let sky;if(skyGroups.size===1){const same=skyRows[0].sky;const variants=[same+" sur l'ensemble des zones suivies",same+" d'est en ouest sur les secteurs observés","une même tendance sur l'ensemble, avec "+same];sky=variants[hash(day+"weather-sky-group")%variants.length]}else{sky=Array.from(skyGroups.entries()).map(([s,cs])=>cs.length>1?s+" de "+cs[0]+" à "+cs[cs.length-1]:s+" vers "+cs[0]).join(", ")}return "Bonjour, ici Jaya avec votre météo nationale sur Technorizon.fr. Aujourd'hui, "+temps+". Côté ciel, "+sky+". "+(wet.length?"Le risque de pluie est plus marqué vers "+wet.slice(0,3).join(", ")+".":"Le risque de pluie reste globalement limité sur les villes suivies.")+" Et pour retrouver la météo détaillée de votre ville, rendez-vous sur Technorizon.fr, rubrique Météo. Très bonne écoute !"}
...
2. Vous êtes à l’écoute de Technorizon, ne bougez surtout pas, on vous envoie la suite…
3. C’était un bon petit son ça ! Ne bougez pas, la suite va taper fort.
4. Vous êtes toujours sur Technorizon, la web-radio préférée des furax… montez l’son…
5. La suite arrive sur Technorizon, la radio qui va réveiller vos tympans…
6. J’espère que ça vous plaît car on n’a pas l’intention de ralentir ! Calez-vous et montez l’son, vous êtes sur Technorizon…
7. Bonsoir à vous si vous venez d’arriver ! J’espère que vous êtes prêts car à partir de maintenant ça va percuter…
8. Vous êtes bien sur Technorizon, le meilleur de la techno, Eurodance et House music des années 90’s à aujourd’hui ! C’est tout de suite Technorizon.fr
9. Si ça c’était pas bon ! Allez, on vous envoie la suite et c’est maintenant sur Technorizon…
10. Et oui ça percute sur Technorizon ! Réveillez vos voisins, c’est fête collective obligatoire avec ce qui arrive maintenant à l’antenne !
11. Y’a pas à dire, c’est du grand DJ sur Technorizon ! Vous aimez le différent ? Vous êtes au bon endroit ! Allez, on vous envoie la suite…
12. Bon, j’espère que le patron n’écoute pas parce qu’à partir de maintenant, on lâche les chevaux ! Soirée furax sur Technorizon.
